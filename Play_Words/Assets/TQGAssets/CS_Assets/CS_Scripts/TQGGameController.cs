...
namespace TriviaQuizGame
{
	...
	public class TQGGameController : MonoBehaviour
	{
		...
		//  A new if that shows the score screen after one category finished.		
		...
		{
			//New code
			category = TQGCategoryGrid.myCategory;
		...
		void UpdateScore()
		{
			...
			if (scoreToVictory > 0 && players[currentPlayer].score >= scoreToVictory)
			{
				StartCoroutine(Victory(0));				
			}
		}
